...
Workflow:
1. For a topic, keyword, party name, or case name without a reporter citation, discover relevant cases with courtlistener_search_case_law.
...
3. Fetch relevant matched clusters with courtlistener_get_cases.
...
export const COURTLISTENER_TOOLS = [
    {
        ...
        function: {
            ...
            description:
                "Search current CourtListener case-law opinions by topic, keyword, party name, or case name. Returns case metadata and cluster IDs, not cite-worthy opinion text. Fetch relevant results with courtlistener_get_cases before relying on or citing them.",
            parameters: {
                type: "object",
                properties: {
                    query: {
                        type: "string",
                        description:
                            "CourtListener search query, such as a legal issue, party name, or case name.",
                    },
                    court: {
                        type: "string",
                        description:
                            "Optional CourtListener court identifier used to limit results, such as scotus or ca9.",
                    },
                    filedAfter: {
                        type: "string",
                        description:
                            "Optional earliest filing date in YYYY-MM-DD format.",
                    },
                    filedBefore: {
                        type: "string",
                        description:
                            "Optional latest filing date in YYYY-MM-DD format.",
                    },
                    limit: {
                        type: "integer",
                        minimum: 1,
                        maximum: 20,
                        description:
                            "Maximum number of search results to return. Default 10; maximum 20.",
                    },
                },
                required: ["query"],
            },
